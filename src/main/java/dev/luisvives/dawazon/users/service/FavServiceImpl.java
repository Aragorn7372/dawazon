...
public class FavServiceImpl implements FavService {
    ...
    @Autowired
    public FavServiceImpl(UserRepository userRepository, ProductRepository productRepository,ProductMapper productMapper) {
                ...
                .map((it)->productMapper.modelToGenericResponseDTO(
                        it,it.getComments().stream()
                                .map((comment)->productMapper.commentToCommentDto(comment,
                                        userRepository.findActiveById(comment.getUserId()).getUsername())).toList())).toList(),
